...
class AuditReport(Document):
	...
	def _build_context(self):
		engagement = frappe.db.get_value(
			"Engagement", self.engagement, ["client", "financial_year", "period_end"], as_dict=True
		) or {}
		client_name = None
		if engagement.get("client"):
			client_name = frappe.db.get_value("Customer", engagement["client"], "customer_name")
		framework = None
		letter = frappe.db.get_value("Engagement Letter", {"engagement": self.engagement}, "reporting_framework")
		if letter:
			framework = frappe.db.get_value("Reporting Framework", letter, "title") or letter
